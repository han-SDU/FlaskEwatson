...
class HistoricPressureModel():
	...
	def delete_by_range(start, end):
		...
		logging.debug("Starting delete")
		cur.execute(
			'Delete from tbl_historic_pressure where fld_start_time>=? and fld_end_time<=?', (start, end,))
	...
	def get_by_search(start, end):
		...
		logging.debug("Starting select")
		cur.execute(
			'Select * from tbl_historic_pressure where fld_start_time>=? and fld_end_time<=?', (start, end,))
	...
	def get_oldest():
		...
		# Execution
		logging.debug("Starting select")
		cur.execute('Select * from tbl_historic_pressure order by fld_end_time asc limit 1')
	...
	def get_newest():
		...
		# Execution
		logging.debug("Starting select")
		cur.execute('Select * from tbl_historic_pressure order by fld_end_time desc limit 1')
	...
	def get_average():
		...
		# Execution
		logging.debug("Starting select")
		cur.execute('Select AVG(fld_average) from tbl_historic_pressure')
	...
	def get_average_by_range(start, end):
		...
		logging.debug("Starting select")
		cur.execute(
			'Select AVG(fld_average) from tbl_historic_pressure where fld_start_time>=? and fld_end_time<=?', (start, end,))
