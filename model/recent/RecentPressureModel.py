...
class RecentPressureModel():
	...
	def to_json(self):
		data = {
			'type': 'Recent pressure sensor reading',
			'id': self.id,
			'attributes': {
	...
	def delete_all():
		...
		cur.execute("Delete from tbl_recent_pressure")
	...
	def delete_by_range(start, end):
		...
		logging.debug("Starting delete")
		cur.execute(
			'Delete from tbl_recent_pressure where fld_time>=? and fld_time<=?', (start, end,))
	...
	def get_all():
		...
		# Execution
		logging.debug("Starting select")
		cur.execute('Select * from tbl_recent_pressure')

		# Formatting of return data
		logging.debug("Formatting query data to objects")
		for id, time, value in cur:
			temp = RecentPressureModel(id, time, value)
	...
	def get_by_search(start, end):
		...
		logging.debug("Starting select")
		cur.execute(
			'Select * from tbl_recent_pressure where fld_time>=? and fld_time<=?', (start, end,))
	...
	def get_oldest():
		...
		# Execution
		logging.debug("Starting select")
		cur.execute('Select * from tbl_recent_pressure order by fld_time asc limit 1')
	...
	def get_average():
		...
		# Execution
		logging.debug("Starting select")
		cur.execute('Select AVG(fld_value) from tbl_recent_pressure')
	...
	def get_average_by_range(start, end):
		...
		logging.debug("Starting select")
		cur.execute(
			'Select AVG(fld_value) from tbl_recent_pressure where fld_time>=? and fld_time<=?', (start, end,))
